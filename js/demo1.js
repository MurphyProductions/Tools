...
var camera = new THREE.PerspectiveCamera(50, width / height, 1.1, 2000);
...
for (var i = 0; i < dotsAmount; i++) {
    ...
    vector.multiplyScalar(125 + (Math.random() + 1.5) * 0);
    vector.scaleX = 5;
    ...
    vector.toArray(positions, i * 30);
    colors[vector.color].toArray(colorsAttribute, i*30);
    sizes[i] = 50;
}
...
function moveDot(vector, index) {
        ...
        TweenMax.to(vector, Math.random() * 40 + 4, {
            x: tempVector.x,
            y: tempVector.y,
            z: tempVector.z,
            yoyo: true,
...
function render(a) {
    var i;
    dotsGeometry.verticesNeedUpdate = false;
